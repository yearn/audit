...
contract StrategyProxy {
    ...
    function harvest(address _gauge) external {
        ...
        uint _fee = _balance.mul(fee).div(BASE);

        proxy.execute(crv, 0, abi.encodeWithSignature("transfer(address,uint256)", msg.sender, _balance.sub(_fee)));

        if (reserve != address(0x0)) {
            proxy.execute(crv, 0, abi.encodeWithSignature("transfer(address,uint256)", reserve, _fee));
        }
    }
}
