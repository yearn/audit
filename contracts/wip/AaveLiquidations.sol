...
contract AaveLiquidate is ReentrancyGuard, Ownable, Structs {
  ...
  address constant public one = address(0x50FDA034C0Ce7a8f7EFDAebDA7Aa7cA21CC1267e);
  ...
  function getMaxDebt(address __user) public view returns (address _reserve, uint256 _amount) {
    uint256 bTUSD = getDebt(TUSD, __user);
    uint256 bUSDT = getDebt(USDT, __user);
    uint256 bUSDC = getDebt(USDC, __user);
    uint256 bDAI = getDebt(DAI, __user);
  ...
  function getDebt(address _reserve, address __user) public view returns (uint256) {
    (,uint256 debt,,,,,,,,) = Aave(aave).getUserReserveData(_reserve, __user);
    return debt;
  }

  function getMaxCollateral(address __user) public view returns (address _reserve, uint256 _amount) {
    // ETH-DAI, ETH-MKR, ETH-USDC, ETH-SNX, ETH-WBTC, ETH-BAT, ETH-LINK, ETH-KNC, ETH-sUSD
    uint256 _dai = getCollateral(DAI, __user, 1);
    uint256 _mkr = getCollateral(MKR, __user, 1);
    uint256 _usdc = getCollateral(USDC, __user, 1e12);
    uint256 _snx = getCollateral(SNX, __user, 1);
    uint256 _wbtc = getCollateral(WBTC, __user, 1e10);
    uint256 _bat = getCollateral(BAT, __user, 1);
    uint256 _link = getCollateral(LINK, __user, 1);
    uint256 _knc = getCollateral(KNC, __user, 1);
    uint256 _susd = getCollateral(SUSD, __user, 1);
  ...
  function getCollateral(address _reserve, address __user, uint256 _dec) public view returns (uint256) {
    (uint256 _collateral, uint256 _debt,,,,,,,,) = Aave(aave).getUserReserveData(_reserve, __user);
  ...
  function getCollateralToLiquidate(address _reserve, address __user) public view returns (uint256) {
    (uint256 _collateral,,,,,,,,,) = Aave(aave).getUserReserveData(_reserve, __user);
    return _collateral;
  }
  ...
  function callFunction(
    ...
    if (_reserve != USDC) {
      ...
      uint _expected;
      (_expected, _distribution) = OneSplitAudit(one).getExpectedReturn(USDC, _reserve, _sell, 1, 0);
      OneSplitAudit(one).swap(USDC, _reserve, _sell, _expected, _distribution, 0);
    ...
    if (_reserve != USDC) {
      ...
      uint[] memory _distribution;
      uint _expected;
      (_expected, _distribution) = OneSplitAudit(one).getExpectedReturn(_reserve, USDC, IERC20(_reserve).balanceOf(address(this)), 1, 0);
      OneSplitAudit(one).swap(_reserve, USDC, IERC20(_reserve).balanceOf(address(this)), _expected, _distribution, 0);
  ...
  function liquidate(address _collateral, address _reserve) public {
    ...
    uint[] memory _distribution;
    uint _expected;
    (_expected, _distribution) = OneSplitAudit(one).getExpectedReturn(_collateral, _reserve, _liquidated, 1, 0);
      OneSplitAudit(one).swap(_collateral, _reserve, _liquidated, _expected, _distribution, 0);
    IERC20(_collateral).safeApprove(one, 0);
  }
