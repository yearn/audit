...
contract StrategySNXSUSD {
    ...
    function debt() public view returns (uint) {
        return snx().debtBalanceOf(address(this), "sUSD");
    }
    ...
    function debtToBurn() public view returns (uint) {
        uint _issuable = maxIssuableSynths();
        uint _debt = debt();
        if (_debt > _issuable) {
            return _debt.sub(_issuable);
        } else {
            return 0;
        }
    }
    ...
    function feesAvailable() public view returns (uint,uint) {
        return fees().feesAvailable(address(this));
    }

    function claimFees() public {
        // Claim weekly fees (if available)
        if (isFeesClaimable()) {
            (uint availableFees, uint availableRewards) = feesAvailable();
