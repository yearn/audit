...
contract yCollateralVault is ERC20, ERC20Detailed {
    ...
    constructor(address _asset) public ERC20Detailed(
        string(abi.encodePacked("yearn ", ERC20Detailed(_asset).name())),
        string(abi.encodePacked("y",ERC20Detailed(_asset).symbol())),
        18
    ) {
    ...
    function borrow(address reserve, uint256 amount, address to) external {
        Aave(getAave()).borrow(reserve, amount, 2, 7);
        IERC20(reserve).safeTransfer(to, amount);
    }

    function repay(address reserve, uint256 amount) external {
