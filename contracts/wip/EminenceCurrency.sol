...
contract ContinuousToken is ERC20 {
    ...
    event ContinuousMint(address _from, uint  _amount, uint _deposit);
    event ContinuousBurn(address _from, uint  _amount, uint _reimbursement);
...
contract EminenceCurrency is ContinuousToken, ERC20Detailed {
    ...
    event AddGameMaster(address indexed newGM, address indexed gm);
    event RevokeGameMaster(address indexed newGM, address indexed gm);

    IERC20 constant public DAI = IERC20(0x6B175474E89094C44Da98b954EedeAC495271d0F);

    constructor (string memory name, string memory symbol, uint _reserveRatio) public ERC20Detailed(name, symbol, 18) {
    ...
    function addGM(address _gm) external {
        ...
        emit AddGameMaster(_gm, msg.sender);
    }
    function revokeGM(address _gm) external {
        require(gamemasters[msg.sender], "!gm");
        gamemasters[_gm] = false;
        emit RevokeGameMaster(_gm, msg.sender);
    }
    function pay(address _to, uint _amount) external {
        require(gamemasters[msg.sender], "!gm");
        _mint(_to, _amount);
    }
    function receive(address _from, uint _amount) external {
        require(gamemasters[msg.sender], "!gm");
        _burn(_from, _amount);
    }
    ...
    function buy(uint _amount, uint _min) external returns (uint _bought) {
        ...
        emit ContinuousMint(msg.sender, _bought, _amount);
    }
    function sell(uint _amount, uint _min) external returns (uint _bought) {
        ...
        emit ContinuousBurn(msg.sender, _amount, _bought);
    }
}
