...
contract HegicERCPool is
    IERCLiquidityPool,
    ERC20("Hegic yv3pool LP Token", "writeYV3pool")
{
    using SafeMath for uint256;
    ...
    function addOption(address _option) external {
        require(msg.sender == governance, "HegicERCPool::addOption: not governance");
        require(!options[_option], "HegicERCPool:addOption: option exists");
        options[_option] = true;
        contracts.push(_option);
    }

    function revokeOption(address _option) external {
        require(msg.sender == governance, "HegicERCPool::addOption: not governance");
        require(options[_option], "HegicERCPool:addOption: option unknown");
        options[_option] = false;
    }

    function setGovernance(address _governance) external {
        require(msg.sender == governance, "HegicERCPool::setGovernance: not governance");
        pendingGovernance = _governance;
    }

    function acceptGovernance() external {
        require(msg.sender == pendingGovernance, "HegicERPool::acceptGovernance: not pending governance");
    ...
    function setLockupPeriod(uint256 value) external override {
        require(msg.sender == governance, "HegicERCPool::setLockupPeriod: not governance");
        require(value <= 60 days, "Lockup period is too large");
        lockupPeriod = value;
    }

    function getNextID() external view returns (uint) {
        return lockedLiquidity.length;
    }
    ...
    function lock(uint id, uint256 amount, uint256 premium) external override {
        ...
        RESERVE.safeTransferFrom(msg.sender, address(this), premium);
    }
     ...
     */
    function send(uint id, address payable to, uint256 amount) external override {
        require(options[msg.sender], "HegicERCPool::setLockupPeriod: not option");
        LockedLiquidity storage ll = lockedLiquidity[id];
        require(ll.locked, "LockedLiquidity with such id has already unlocked");
        ...
        RESERVE.safeTransfer(to, transferAmount);
    ...
    /*
     * @nonce A provider supplies RESERVE to the pool and receives WRITER tokens
    ...
    /*
     * @nonce Provider burns WRITER and receives RESERVE from the pool
     * @param amount Amount of RESERVE to receive
    ...
    function withdraw(uint256 amount, uint256 maxBurn) external returns (uint256 burn) {
        ...
        require(RESERVE.transfer(msg.sender, amount), "Insufficient funds");
    }

    /*
     * @nonce Returns provider's share in RESERVE
     * @param account Provider's address
     * @return Provider's share in RESERVE
     */
    ...
    /*
     * @nonce Returns the RESERVE total balance provided to the pool
     * @return balance Pool balance
     */
    function totalBalance() public override view returns (uint256 balance) {
        return RESERVE.balanceOf(address(this)).sub(lockedPremium);
    }
