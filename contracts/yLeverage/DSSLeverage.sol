...
interface saver {
  function getCdpDetailedInfo(uint _cdpId) external view returns (uint collateral, uint debt, uint price, bytes32 ilk);
}
...
contract DSSLeverage is ReentrancyGuard, Structs, Ownable, DssProxyActions  {
  ...
  address public constant save = 0x1637d405F61A6C6AA54d0a28faB85B7eA9e81932;
  ...
  function isLeverage(uint256 _leverage) public pure returns (bool) {
    if (_leverage == 2) {
      return true;
    } else if (_leverage == 3) {
      return true;
  ...
  function leverage(uint256 _amount, uint256 _leverage) public nonReentrant {
    require(isLeverage(_leverage) == true, "leverage");
    IERC20(usdc).safeTransferFrom(msg.sender, address(this), _amount);

    caller = msg.sender;
    cdp = 0;
    position = _amount.mul(_leverage);
    wadC = 0;
  ...
  function close(uint256 _cdp) public nonReentrant {
    (uint collateral, uint debt,,) = saver(save).getCdpDetailedInfo(_cdp);
    _settle(debt, _cdp, collateral.div(1e12));
  }
  function settle(uint256 _dai, uint256 _cdp) public nonReentrant {
    uint256 _usdc = _dai.div(1e12).mul(110).div(100);
    _settle(_dai, _cdp, _usdc);
  }

  function _settle(uint256 _amount, uint256 _cdp, uint256 _usdc) internal {
    cdp = _cdp;
    position = 0;
    caller = address(0x0);
    wadC = _usdc;
