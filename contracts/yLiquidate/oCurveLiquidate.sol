...
contract oCurveLiquidate is ReentrancyGuard, Ownable {
  using SafeERC20 for IERC20;
  using Address for address;
  using SafeMath for uint256;

  address constant public oyCurve = address(0x4BA8C6Ce0e855C051e65DfC37883360efAf7c82B);

  function liquidate(address payable _vault, address payable _oToken) public {

    OptionsContract oToken = OptionsContract(_oToken);
    require(oToken.isUnsafe(_vault), 'safe');

    uint256 maxToLiquidate = oToken.maxOTokensLiquidatable(_vault); //100 * 1e15

    IERC20(_oToken).transferFrom(msg.sender, address(this), maxToLiquidate);
