...
contract AaveLiquidate is ReentrancyGuard, Ownable, Structs {
  ...
  function liquidate(address _victim) external nonReentrant {
    ...
    if (_reserve == DAI||_reserve == TUSD) {
      _debt = _debt.div(1e12);
    }
  ...
  function getCurveID(address _reserve) public pure returns (int128) {
    if (_reserve == DAI) {
      return 0;
    } else if (_reserve == USDT) {
      return 2;
  ...
  function callFunction(
    ...
    if (_reserve != USDC) {
      ...
      CurveFi(curve).exchange_underlying(1, getCurveID(_reserve), IERC20(USDC).balanceOf(address(this)), 1);
    ...
    if (_reserve != USDC) {
      IERC20(_reserve).safeApprove(curve, uint256(-1));
      CurveFi(curve).exchange_underlying(getCurveID(_reserve), 1, IERC20(_reserve).balanceOf(address(this)), 1);
      IERC20(_reserve).safeApprove(curve, 0);
    }
  }
